...
def round_percent(x): return round(float(x) * 100, 2)
...
class PortfolioSummaryInstance:  # pragma: no cover
    ...
    def __init__(self, ledger, config, irr_cache, dividend_cache, filtered):
        ...
        self.irr = IRR(self.ledger.all_entries,
                       g.ledger.price_map,
        ...
        self.all_cols = [
            "units", "cost", "balance", "pnl", "dividends", "change", "mwr",
            "twr", "allocation"
        ]
    ...
    def run(self):
        ...
        for res in self._parse_config():
            if len(res) == 1:
                cols = [c for c in res[0] if c in seen_cols]
                break

            key, pattern, internal, cols, mwr, twr = res
            seen_cols.update({c: None for c in cols})
        ...
        if 'mwr' in seen_cols or 'twr' in seen_cols:
            self.total['mwr'], self.total['twr'] = self._calculate_irr_twr(
                self.all_mwr_accounts, all_mwr_internal, 'mwr' in seen_cols,
                'twr' in seen_cols)

        portfolios = [("All portfolios",
                       (self._get_types(cols), [self.total]))] + portfolios
        print(
            f"Done: Elapsed: {time.time() - _t0:.2f} (mwr/twr: {self.irr.elapsed():.2f}, "
    ...
    def _parse_config(self):
        ...
        if not isinstance(self.config, dict):
            raise FavaAPIException(
                "Portfolio List: Config should be a dictionary.")

        for key in ('metadata-key', 'account-groups'):
            if key not in self.config:
                raise FavaAPIException(
                    f"Portfolio List: '{key}' is required key.")

        for key in self.config:
            if key not in keys:
                raise FavaAPIException(
                    f"Portfolio List: '{key}' is an invalid key.")
        ...
        if isinstance(internal, (tuple, list)):
            ...
            raise FavaAPIException(
                "Portfolio List: 'internal' must be a list.")
        ...
        dividends = self.config.get('dividends', 'dividends'
                                    in self.config.get('cols', []))

        if isinstance(mwr, str) and mwr != "children":
            raise FavaAPIException(
                "Portfolio List: 'mwr' must be one of (True, False, 'children')"
            )
        if isinstance(twr, str) and twr != "children":
            raise FavaAPIException(
                "Portfolio List: 'twr' must be one of (True, False, 'children')"
            )
        if isinstance(dividends, str):
            raise FavaAPIException(
                "Portfolio List: 'dividends' must be one of (True, False)")

        for group in self.config['account-groups']:
            yield self.config['metadata-key'], *self._parse_group(
                group, internal, cols, mwr, twr, dividends)
    ...
    def _parse_group(self, group, internal, cols, mwr, twr, dividends):
        ...
        if isinstance(group, dict):
            try:
                ...
                grp_dividends = group.get(
                    'dividends', dividends if 'dividends' in cols else False)
                for col in cols:
                    if col not in self.all_cols:
                        raise FavaAPIException(
                            f"Portfolio List: '{col}' is not a valid column. "
    ...
    def _get_types(cols):
        ...
        return [(c, col_map[c]) for c in cols]

    def _account_metadata_pattern(self, tree, key, pattern, internal,
                                  mwr, twr, dividends):
        """
        ...
        for entry in accounts:
            ...
            if (key in entry.meta) and regexer.match(entry.meta[key]):
                selected_accounts.append({
                    'account': tree[entry.account],
                    'children': []
                })
        ...
        portfolio_data = self._portfolio_data(selected_accounts, internal, mwr,
                                              twr, dividends)
        ...
        if len(result[2]) > 0:
            for row_cost in result[2]:
                if len(row_cost.dividends.get_positions()) == 1:
                    dividends += round(
                        abs(row_cost.dividends.get_positions()
                            [0].units.number), 2)
    ...
    def _process_node(self, node, dividends):
        ...
        units = cost_or_value(node.balance, "units", price_map, date=date)
        ...
        if dividends:
            if row_currency is not None and row_currency not in self.ledger.options[
                    "operating_currency"]:
        ...
        elif (row_currency is not None
              and self.operating_currency not in balance
    ...
    def _portfolio_data(self, nodes, internal, mwr, twr, dividends):
        ...
        for node in nodes:
            ...
            for child in node['children']:
                ...
                if mwr == "children" or twr == "children":
                    row['mwr'], row['twr'] = self._calculate_irr_twr(
                        [row['account']], internal, mwr == "children",
                        twr == "children")
            ...
            if mwr or twr:
                ...
                parent['mwr'], parent['twr'] = self._calculate_irr_twr(
                    [pattern], internal, mwr, twr)
        ...
        if mwr or twr:
            total['mwr'], total['twr'] = self._calculate_irr_twr(
                mwr_accounts, internal, mwr, twr)
